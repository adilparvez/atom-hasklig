...
const handleFontWeightChange = weight => {
  document.body.setAttribute('data-font', `hasklig-${weight}`)
}
...
const handleScopesChange = scopes => {
  const styleSheet = getOrCreateStyleSheet('hasklig')
...
export const loadPackage = () => {
  atom.config.observe('hasklig.fontWeight', handleFontWeightChange)
  atom.config.observe('hasklig.scopes', handleScopesChange)
}
